      ...
      <div className="flex flex-col min-h-screen bg-[#FDFBF9] animate-in fade-in duration-700">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto w-full">
          ...
          <div className="lg:col-span-8 space-y-6 md:space-y-8">
            ...
            <section className="relative rounded-[2rem] overflow-hidden aspect-[2/1] sm:aspect-[3.5/1] shadow-xl group">
              {HERO_SLIDES.map((slide, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/30 to-transparent flex flex-col justify-center px-6 md:px-12 text-white">
                    <h1 className="text-xl md:text-2xl font-bold font-headline max-w-md leading-tight animate-in slide-in-from-left-4 duration-700">
                      {slide.title}
                    </h1>
                    <p className="mt-2 md:mt-4 text-[10px] md:text-xs opacity-90 max-w-xs md:max-w-sm">
                      {slide.description}
                    </p>
                  </div>
                </div>
              ...
              <div className="hidden md:flex absolute bottom-8 left-12 right-12 z-20 items-center bg-white rounded-2xl p-1 shadow-2xl max-w-xl mx-auto">
                ...
                <Button className="bg-primary hover:bg-primary/90 rounded-xl px-8 font-bold h-11 text-xs">Search</Button>
              </div>
            </section>
              ...
              {[
                ...
                <Link key={i} href={item.href} className="group shrink-0 sm:shrink">
                  <div className="flex flex-col items-center gap-1 md:gap-2 p-1 md:p-2 rounded-2xl md:rounded-3xl transition-all hover:bg-white hover:shadow-md">
                    <div className={cn("h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg", item.color)}>
                      <item.icon className="h-5 w-5 md:h-6 md:w-6" />
                    </div>
                    <div className="text-center">
                      <p className="text-[8px] font-bold uppercase tracking-tight md:text-[9px] text-slate-700">{item.title}</p>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              <Card className="bg-[#2D4A22] text-white border-none rounded-[2rem] overflow-hidden p-5 relative group cursor-pointer shadow-md">
                <div className="relative z-10 space-y-3">
                  <h3 className="text-base md:text-lg font-bold font-headline leading-tight">Healthy Pets Happier Lives</h3>
                  <p className="text-[9px] md:text-[10px] opacity-80">Book grooming, training, boarding and more.</p>
                  <Button variant="secondary" size="sm" className="rounded-xl font-bold h-7 text-[9px] md:text-[10px]">Explore Services <ArrowRight className="ml-2 h-3 w-3" /></Button>
                </div>
                <Image src="https://picsum.photos/seed/promo1/300/300" alt="Dog" width={100} height={100} className="absolute bottom-0 right-0 object-cover opacity-50 group-hover:scale-110 transition-transform md:w-[120px] md:h-[120px]" />
              </Card>

              <Card className="bg-[#E9B7CE] text-[#802D52] border-none rounded-[2rem] overflow-hidden p-5 relative group cursor-pointer shadow-md">
                <div className="relative z-10 space-y-3">
                  <h3 className="text-base md:text-lg font-bold font-headline leading-tight">Every Pet Deserves Love</h3>
                  <p className="text-[9px] md:text-[10px] opacity-80">Join our adoption network today.</p>
                  <Button size="sm" className="bg-[#802D52] text-white rounded-xl font-bold h-7 text-[9px] md:text-[10px]">Explore Adoption <ArrowRight className="ml-2 h-3 w-3" /></Button>
                </div>
                <Image src="https://picsum.photos/seed/promo2/300/300" alt="Pets" width={100} height={100} className="absolute bottom-0 right-0 object-cover opacity-50 group-hover:scale-110 transition-transform md:w-[120px] md:h-[120px]" />
              </Card>

              <Card className="bg-[#0D2B2B] text-white border-none rounded-[2rem] overflow-hidden p-5 relative group cursor-pointer shadow-md">
                <div className="relative z-10 space-y-3">
                  <div className="flex items-center gap-2">
                    <PawPrintIcon className="h-4 w-4 text-primary" />
                    <h3 className="text-base md:text-lg font-bold font-headline leading-tight text-white">Pet Reports</h3>
                  </div>
                  <p className="text-[9px] md:text-[10px] opacity-80">Track health, activity, and habits.</p>
                  <Button variant="secondary" size="sm" className="rounded-xl font-bold h-7 text-[9px] md:text-[10px]">View Reports <ArrowRight className="ml-2 h-3 w-3" /></Button>
                </div>
                <Image src="https://picsum.photos/seed/promo3/300/300" alt="Reports" width={100} height={100} className="absolute bottom-0 right-0 object-cover opacity-50 group-hover:scale-110 transition-transform md:w-[120px] md:h-[120px]" />
              </Card>
            </section>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between px-2">
                <h2 className="text-base md:text-lg font-bold font-headline flex items-center gap-2">
                  <PawPrintIcon className="h-5 w-5 text-primary" /> Featured Services
                </h2>
                <Link href="/providers" className="text-[9px] md:text-[10px] font-bold text-primary flex items-center gap-1">
            ...
            {/* Trust Bar */}
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-dashed">
              {[
                ...
                <div key={i} className="flex items-center gap-2 md:gap-3">
                  <div className="h-7 w-7 md:h-9 md:w-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500">
                    <item.icon className="h-3.5 w-3.5 md:h-4.5 md:w-4.5" />
                  </div>
                  <div>
                    <p className="text-[7px] md:text-[9px] font-bold uppercase tracking-tight">{item.label}</p>
                    <p className="text-[6px] md:text-[7px] text-muted-foreground">{item.text}</p>
                  </div>
                </div>
              ))}
            </section>
          ...
          <div className="lg:col-span-4 space-y-6 md:space-y-8">
            ...
            <Card className="border-none shadow-sm rounded-3xl p-5 md:p-6 bg-white space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Your Pet's Profile</h3>
                <Link href="/pet-profile">
                  <div className="flex items-center gap-3 p-3 rounded-2xl border border-primary/10 bg-primary/5 shadow-sm group hover:border-primary/30 transition-all">
                    <Avatar className="h-10 w-10 md:h-12 md:w-12 rounded-xl">
                      <AvatarImage src={petData?.avatar || "https://picsum.photos/seed/petavatar/100/100"} />
                      <AvatarFallback><Dog /></AvatarFallback>
                    </Avatar>
                    <div className="flex-1 overflow-hidden">
                      <p className="text-sm font-bold truncate text-slate-800">{petData?.name || 'Buddy'}</p>
                      <p className="text-[10px] text-muted-foreground truncate">{petData?.breed || 'Golden Retriever'} • {petData?.age || '2 years'}</p>
                    </div>
            ...
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-center justify-between px-2">
                <h3 className="font-bold text-sm md:text-base">Upcoming Sessions</h3>
                <Link href="/bookings" className="text-[10px] font-bold text-primary">View All</Link>
              </div>
              <div className="space-y-3">
                {upcomingBooking ? (
            ...
            {pendingRequests.length > 0 && (
              <div className="space-y-3 md:space-y-4">
                <div className="flex items-center justify-between px-2">
                  <h3 className="font-bold text-sm md:text-base">Match Requests</h3>
                  <Link href="/match" className="text-[10px] font-bold text-primary">View All</Link>
                </div>
                <div className="space-y-3">
            ...
            <Card className="bg-[#274E4A] text-white border-none rounded-[2rem] overflow-hidden p-6 relative group shadow-lg">
               <div className="relative z-10 space-y-4">
                  <h3 className="text-lg font-bold font-headline leading-tight">Pet Parent Community</h3>
               ...
               <Image src="https://picsum.photos/seed/community/400/400" alt="Community" width={150} height={150} className="absolute -bottom-8 -right-12 object-cover opacity-30 group-hover:scale-110 transition-transform" />
            </Card>

            {/* Recently Viewed / Tips */}
            <div className="space-y-3">
              <h3 className="font-bold text-[10px] text-slate-500 uppercase tracking-widest px-2">Parenting Tips</h3>
              {[
